...
from functions.FDGR import fdgr_diffusion
    ...
    def sample(self, logger):
        ...
        if self.config.model.type == 'simple':
            ...
            if self.config.model.class_cond:
                ckpt = os.path.join(self.args.exp, '' % (
                ...
                ckpt = os.path.join(self.args.exp, "logs/imagenet/256x256_diffusion_uncond.pt") # 是在这里
                if not os.path.exists(ckpt):
                    raise ValueError(
                        "ImageNet model checkpoint not found, please download it as mentioned in README.md"
                        " file and configure the correct path")
        ...
        self.fdgr_wrapper(model, cls_fn, logger)
        
        

    def fdgr_wrapper(self, model, cls_fn, logger):
        ...
        for x_orig, classes in pbar:
            ...
            with torch.no_grad():           
                x, _ = fdgr_diffusion(x, model, self.betas, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config, args=args)
